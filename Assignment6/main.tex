...
The first order statistic for any sample is the first term in the sample once the terms have been placed in ascending order (i.e.. the minimum of the given sample) 
In order to calculate the distribution for $Y_1$, we need the the cumulative distribution function of x:\\
Let $F(x)$ denote the cdf 
\begin{align}
...
\begin{align}
    ...
    \end{cases}
\end{align}
The cdf $F(x)$ represents the probability that a random variable has a value lesser than $x$. Consequently, $1-F(x)$ represents the probability that a random variable has a value greater than or equal to $x$.\\
